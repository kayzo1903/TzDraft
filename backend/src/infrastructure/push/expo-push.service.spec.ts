...
function mockFetch(responseBody: object, status = 200): jest.SpyInstance {
  ...
  describe('sendToUser', () => {
    ...
    it('sends one push message when user has a token', async () => {
      const prisma = makePrisma({
        findUnique: jest
          .fn()
          .mockResolvedValue({ pushToken: 'ExponentPushToken[abc]' }),
      });
      const fetchSpy = mockFetch({ data: [{ status: 'ok', id: 'ticket-1' }] });
      const svc = makeSvc(prisma);

      await svc.sendToUser('user-1', 'Match ready', 'Your turn', {
        matchId: 'm1',
      });
    ...
    it('invalidates the DB token when Expo returns DeviceNotRegistered', async () => {
      const prisma = makePrisma({
        findUnique: jest
          .fn()
          .mockResolvedValue({ pushToken: 'ExponentPushToken[stale]' }),
    ...
    it('does not invalidate token for other Expo error types', async () => {
      const prisma = makePrisma({
        findUnique: jest
          .fn()
          .mockResolvedValue({ pushToken: 'ExponentPushToken[ok]' }),
        updateMany: jest.fn(),
      });
      mockFetch({
        data: [
    ...
    it('swallows network errors without throwing', async () => {
      ...
      await expect(
        svc.sendToUser('user-1', 'Hi', 'World'),
