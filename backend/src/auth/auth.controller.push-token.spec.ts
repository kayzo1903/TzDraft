...
jest.mock('@nestjs/throttler', () => ({ Throttle: () => () => undefined }), { virtual: true });
...
jest.mock('./dto', () => ({}));
jest.mock('./decorators/public.decorator',       () => ({ Public: () => () => undefined }));
jest.mock('./decorators/current-user.decorator', () => ({ CurrentUser: () => () => undefined }));
...
describe('AuthController — push token endpoints', () => {
  function makeController(userServiceOverrides: Partial<{
  ...
  describe('savePushToken', () => {
    it('calls userService.savePushToken with the user id and token', async () => {
      ...
      await controller.savePushToken({ id: 'user-1' }, 'ExponentPushToken[abc]');
    ...
    it('returns undefined (204 No Content body)', async () => {
      ...
      const result = await controller.savePushToken({ id: 'user-1' }, 'ExponentPushToken[xyz]');
