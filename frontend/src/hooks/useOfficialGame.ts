...
export interface OfficialGameState {
  board: BoardState;
...
const DEFAULT_STORAGE_KEY = "tzdraft:official-test";

type UseOfficialGameOptions = {
  storageKey?: string;
};
...
export const useOfficialGame = (
  aiLevel: number,
  playerColor: PlayerColor,
  timeSeconds: number,
  options?: UseOfficialGameOptions,
) => {
  ...
  const initialCurrentPlayer = loaded ? loaded.currentPlayer : PlayerColor.WHITE;
  ...
  const updateEndgameCountdown = useCallback(
    (nextBoard: BoardState, nextRuleState: OfficialRuleState) => {
      ...
      const remaining = Math.max(0, 12 - nextRuleState.imbalanceMovesByStronger);
  ...
  }, [aiLevel, applyMove, board, currentPlayer, playerColor, result, ruleState]);
