    ...
    <div className="divide-y divide-neutral-800/60">
      {entries.map((entry) => {
        ...
        return (
          <div
            key={entry.userId}
            className="grid grid-cols-[4rem_1fr_auto_auto] gap-4 px-5 py-4 items-center hover:bg-white/[0.03] transition-colors"
          >
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold truncate text-neutral-100">{entry.displayName}</p>
            ...
            <div className="text-sm font-black text-neutral-100 text-right min-w-[3.5rem]">{entry.rating}</div>
          </div>
        );
      })}
    </div>
...
export default function LeaderboardClient({
  ...
  // Reload when scope changes (but not on first render — use server data for global p0)
  useEffect(() => {
    if (!initialized) {
      setInitialized(true);
      return;
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <section className="rounded-2xl border border-neutral-800 bg-neutral-900/40 shadow-2xl overflow-hidden">
          ...
          {loading ? (
            ...
            <>
              {/* Highlight logged-in user's row client-side */}
              <div className="divide-y divide-neutral-800/60">
                {entries.map((entry) => {
