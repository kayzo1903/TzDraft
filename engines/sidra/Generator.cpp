...
// AddPromoCaptures is DEAD CODE — intentionally not called.
//
// It would allow a promoted man to continue capturing as a king mid-sequence,
// which is ILLEGAL under TZD rules (Article 4.10 / Article 13.3):
//   "If a man reaches the opponent's back rank during a capture sequence,
//    the capture sequence ENDS immediately."
//
// The correct TZD behaviour is implemented in ManCapture() and TryManCapture(),
// which set promotion=true and terminate the sequence without calling this function.
// This function is kept only for historical reference.
//
...
void AddPromoCaptures(unsigned sq, Move *t, unsigned caps, int dir)
{
	...
	if (OP_CHECKER(m) && !Board[to])
	{
		AddCaptured(m, t, caps);
		Board[m] ^= CHANGE_COLOR;
		AddKingCaptures(to, t, caps + 1, dir); // WRONG: continues as king (TZD violation)
		Board[m] ^= CHANGE_COLOR;
		return;
	}
	CopyMoveFromTemplate(t, caps);
...
inline void ManCapture(unsigned sq, Move *t, unsigned caps, int dir, int bad_dir, bool &found)
{
	...
	{
		// TZD rule: promotion during capture ENDS the capture sequence immediately.
		t->promotion = true;
		CopyMoveFromTemplate(t, caps + 1);
		MP->to = to;
		MP++;
		t->promotion = false;
	}
...
void AddManCaptures(unsigned sq, Move *t, unsigned caps, int bad_dir)
{
	...
	static const int ForwardDirections[2][2] = {{-4, -5}, {4, 5}};
	ManCapture(sq, t, caps, ForwardDirections[stm - 1][0], bad_dir, found);
	ManCapture(sq, t, caps, ForwardDirections[stm - 1][1], bad_dir, found);
	if (!found)
	{
		CopyMoveFromTemplate(t, caps);
		MP->to = sq;
...
inline void TryManCapture(unsigned sq, int dir)
{
	...
	if (to < 9 && stm == WHITE || to > 35 && stm == BLACK)
	{
		// TZD rule: promotion during capture ends the sequence.
		t.promotion = true;
		CopyMoveFromTemplate(&t, 1);
		MP->to = to;
		MP++;
	}
	else
	...
	for (unsigned sq = 5; sq < 40; ++sq)
	{
		if (Board[sq] & stm)
		{
			...
			else
			{
				// TZD rule: men capture forward only (never backward).
				static const int ForwardDirections[2][2] = {{-4, -5}, {4, 5}};
				TryManCapture(sq, ForwardDirections[stm - 1][0]);
				TryManCapture(sq, ForwardDirections[stm - 1][1]);
			}
		}
	}
}
