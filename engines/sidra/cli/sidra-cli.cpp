...
// Define API_CALL if not defined
#ifndef API_CALL
#ifdef _WIN32
...
// Static linking declarations
#ifdef SIDRA_STATIC
extern "C" {
void API_CALL EI_Initialization(void *, int);
void API_CALL EI_NewGame();
void API_CALL EI_SetupBoard(char *);
void API_CALL EI_SetTimeControl(int, int);
void API_CALL EI_SetTime(int, int);
char *API_CALL EI_Think();
...
static int positionFromSquare(const std::string &sq) {
  ...
  if (file < 'a' || file > 'h' || rank < '1' || rank > '8')
    return -1;
  int col = file - 'a';
  int row = '8' - rank; // Match engine's Rank 8 = Row 0
  if ((row + col) % 2 == 0)
    return -1;
...
static std::string resolveDllPath() {
  ...
  std::string exePath(pathBuffer, len);
  size_t slash = exePath.find_last_of("\\/");
  std::string dir =
      (slash == std::string::npos) ? "." : exePath.substr(0, slash);

  const char *env = std::getenv("SIDRA_DLL_PATH");
  if (env && env[0]) {
    return std::string(env);
  }

  std::string dll1 = dir + "\\SiDra.dll";
  std::ifstream f1(dll1.c_str());
  if (f1.good())
    return dll1;

  return dir + "\\sidra.dll";
}
#endif
...
int main() {
  ...
  const int timeLimitMs = getIntField(input, "timeLimitMs", 5000);
...
#ifdef SIDRA_STATIC
  // Static linking: use declared functions directly
  PF_Init EI_Initialization_Ptr = EI_Initialization;
  PF_NewGame EI_NewGame_Ptr = EI_NewGame;
  PF_SetupBoard EI_SetupBoard_Ptr = EI_SetupBoard;
  PF_SetTimeControl EI_SetTimeControl_Ptr = EI_SetTimeControl;
  PF_SetTime EI_SetTime_Ptr = EI_SetTime;
  PF_Think EI_Think_Ptr = EI_Think;
#else
  // Dynamic linking
  const std::string dllPath = resolveDllPath();
  HMODULE dll = LoadLibraryA(dllPath.c_str());
  if (!dll) {
    std::cerr << "Failed to load SiDra DLL: " << dllPath << "\n";
    return 1;
  }

  PF_Init EI_Initialization_Ptr =
      (PF_Init)GetProcAddress(dll, "EI_Initialization");
  PF_NewGame EI_NewGame_Ptr = (PF_NewGame)GetProcAddress(dll, "EI_NewGame");
  PF_SetupBoard EI_SetupBoard_Ptr =
      (PF_SetupBoard)GetProcAddress(dll, "EI_SetupBoard");
  PF_SetTimeControl EI_SetTimeControl_Ptr =
      (PF_SetTimeControl)GetProcAddress(dll, "EI_SetTimeControl");
  PF_SetTime EI_SetTime_Ptr = (PF_SetTime)GetProcAddress(dll, "EI_SetTime");
  PF_Think EI_Think_Ptr = (PF_Think)GetProcAddress(dll, "EI_Think");

  if (!EI_Initialization_Ptr || !EI_NewGame_Ptr || !EI_SetupBoard_Ptr ||
      !EI_SetTimeControl_Ptr || !EI_SetTime_Ptr || !EI_Think_Ptr) {
    std::cerr << "Missing required SiDra exports\n";
    FreeLibrary(dll);
    return 1;
  }
#endif

  EI_Initialization_Ptr(nullptr, 64);
  EI_NewGame_Ptr();
  EI_SetupBoard_Ptr(const_cast<char *>(setup.data()));
  EI_SetTimeControl_Ptr(0, 0);
  EI_SetTime_Ptr(timeLimitMs, timeLimitMs);

  // Silence engine's cout output during search
  std::streambuf *originalCoutPath = std::cout.rdbuf();
#ifdef _WIN32
  ...
  const char *moveStr = EI_Think_Ptr();

  std::cout.rdbuf(originalCoutPath);
  if (!moveStr || moveStr[0] == '\0') {
    std::cout << "{\"from\":-1,\"to\":-1,\"capturedSquares\":[],"
                 "\"isPromotion\":false}";
#ifndef SIDRA_STATIC
    FreeLibrary(dll);
#endif
...
#if defined(_DEBUG)
  std::cerr << "[sidra-cli] moveStr: " << move << "\n";
#endif
  std::vector<std::string> segments = splitMove(move);
#if defined(_DEBUG)
  std::cerr << "[sidra-cli] segments:";
  ...
  std::cout << out.str();
#if defined(_DEBUG)
  std::cerr << "[sidra-cli] capturedSquares:";
  for (size_t i = 0; i < capturedSquares.size(); i++) {
    std::cerr << " " << capturedSquares[i];
  }
  std::cerr << "\n";
#endif
